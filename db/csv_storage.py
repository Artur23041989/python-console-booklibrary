...
class CSVStorage:
    def __init__(self, filename):
        ...
        if file_exist:                         # если файл существует при открытии программы то
            self.last_id = self.get_last_id()  # последний id будет равен результату метода get_last_id()
        else:                                  # иначе, если файл не существует
            self.last_id = "0"                 # последний id будет равен 0 ! для того, чтобы первая строчка была с 1

        self.file = open(filename, "a+", encoding='utf-8')          # открывается файл в режиме дозаписи
    ...
    def get_last_id(self):
        data = self.read_data()
        if data:             # если 'data' есть, т.е. файл не пустой, то:
